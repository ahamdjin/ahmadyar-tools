...
type PageId = 'baseline' | 'shape' | 'cost' | 'result'
...
function NumberInput({ value, onChange, min = 0, max, step = 1, suffix }: { value: number; onChange: (value: number) => void; min?: number; max?: number; step?: number; suffix?: string }) {
  return <label className="relative block"><input type="number" value={value} min={min} max={max} step={step} onChange={(event) => onChange(Number(event.target.value))} className={`min-h-12 w-full rounded-lg border border-zinc-300 bg-white px-3.5 py-2.5 text-sm font-medium text-zinc-950 outline-none focus:border-zinc-950 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-50 dark:focus:border-zinc-100 ${suffix ? 'pr-12' : ''}`} />{suffix ? <span className="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 text-xs text-zinc-500">{suffix}</span> : null}</label>
}

function Field({ label, helper, children }: { label: string; helper?: string; children: React.ReactNode }) {
  return <div className="grid gap-3 py-4 sm:grid-cols-[210px_1fr] sm:gap-7"><div><p className="text-[15px] font-medium leading-6 text-zinc-950 dark:text-zinc-50">{label}</p>{helper ? <p className="mt-1 text-[13px] leading-5 text-zinc-500 dark:text-zinc-400">{helper}</p> : null}</div><div className="self-center">{children}</div></div>
}
...
function BaselineForm({ input, update }: { input: AutomationRoiInput; update: <K extends keyof AutomationRoiInput>(key: K, value: AutomationRoiInput[K]) => void }) {
  return <div className="space-y-5">
    <header><p className="text-sm font-medium text-zinc-950 dark:text-zinc-50">Start with the work before automation.</p><h2 className="mt-2 text-2xl font-medium tracking-[-0.035em] text-zinc-950 sm:text-3xl dark:text-zinc-50">What does the process cost today?</h2><p className="mt-3 text-[15px] leading-7 text-zinc-600 dark:text-zinc-400">Use a representative month. The calculator separates staff capacity from real captured value so “time saved” does not automatically become fake cash savings.</p></header>
    ...
    <Field label="Loaded labor cost / hour" helper="Salary plus payroll burden or the real blended internal cost of the people doing the work."><NumberInput value={input.loadedHourlyCost} onChange={(value) => update('loadedHourlyCost', value)} min={0} step={1} /></Field>
    <Field label="How solid is this baseline?"><Select value={input.baselineSource} onChange={(value) => update('baselineSource', value as RoiBaselineSource)}><option value="measured">Measured from real work</option><option value="estimated">Reasonable operational estimate</option><option value="guess">Mostly a guess</option></Select></Field>
    ...
    <Field label="Cost of one avoidable error" helper="Use zero if errors are annoying but do not have a defensible cost."><NumberInput value={input.costPerError} onChange={(value) => update('costPerError', value)} min={0} step={1} /></Field>
  </div>
}

function ShapeForm({ input, update }: { input: AutomationRoiInput; update: <K extends keyof AutomationRoiInput>(key: K, value: AutomationRoiInput[K]) => void }) {
  return <div className="space-y-5">
    <header><p className="text-sm font-medium text-zinc-950 dark:text-zinc-50">Now model the operating reality.</p><h2 className="mt-2 text-2xl font-medium tracking-[-0.035em] text-zinc-950 sm:text-3xl dark:text-zinc-50">How much work really disappears?</h2><p className="mt-3 text-[15px] leading-7 text-zinc-600 dark:text-zinc-400">Automation coverage, human review and exceptions matter more than a glossy “90% automated” claim.</p></header>
    <Field label="Automatable share" helper="The portion that can follow a repeatable rule or workflow without redesigning the whole process."><NumberInput value={input.automatablePct} onChange={(value) => update('automatablePct', value)} min={0} max={100} suffix="%" /></Field>
    ...
    <Field label="Exception / fallback rate" helper="Cases that automation cannot finish and hands back to a person."><NumberInput value={input.exceptionPct} onChange={(value) => update('exceptionPct', value)} min={0} max={100} suffix="%" /></Field>
    <Field label="Value capture" helper="What share of returned staff capacity becomes real economic value: avoided hiring, more output, redeployment, or actual cost reduction?"><NumberInput value={input.valueCapturePct} onChange={(value) => update('valueCapturePct', value)} min={0} max={100} suffix="%" /></Field>
...
function CostForm({ input, update }: { input: AutomationRoiInput; update: <K extends keyof AutomationRoiInput>(key: K, value: AutomationRoiInput[K]) => void }) {
  ...
  return <div className="space-y-6">
    <header><p className="text-sm font-medium text-zinc-950 dark:text-zinc-50">Price the system you will actually own.</p><h2 className="mt-2 text-2xl font-medium tracking-[-0.035em] text-zinc-950 sm:text-3xl dark:text-zinc-50">What does build + maintenance cost?</h2><p className="mt-3 text-[15px] leading-7 text-zinc-600 dark:text-zinc-400">A cheap subscription can still be expensive to operate. Include build effort, software and recurring maintenance.</p></header>
    <Field label="One-time build cost"><NumberInput value={input.buildCost} onChange={(value) => update('buildCost', value)} min={0} step={100} /></Field>
    ...
    <Field label="Maintenance labor cost / hour"><NumberInput value={input.maintenanceHourlyCost} onChange={(value) => update('maintenanceHourlyCost', value)} min={0} step={1} /></Field>
    <div className="rounded-2xl bg-zinc-100 p-5 dark:bg-zinc-900"><p className="text-xs font-medium uppercase tracking-[0.12em] text-zinc-500">Live read</p><p className="mt-2 text-lg font-medium tracking-[-0.025em] text-zinc-950 dark:text-zinc-50">{preview.dispositionLabel}</p><p className="mt-2 text-sm leading-6 text-zinc-600 dark:text-zinc-400">Current confidence {preview.confidence}%. This can still change when cost assumptions change; the result also stress-tests a deliberately worse case.</p></div>
  </div>
}
...
function Metric({ label, value, detail }: { label: string; value: string; detail?: string }) {
  return <div className="rounded-2xl bg-zinc-100 p-4 dark:bg-zinc-900"><p className="text-xs text-zinc-500">{label}</p><p className="mt-1 text-xl font-medium tracking-[-0.03em] text-zinc-950 dark:text-zinc-50">{value}</p>{detail ? <p className="mt-1 text-[11px] leading-4 text-zinc-500">{detail}</p> : null}</div>
}
...
function Result({ input }: { input: AutomationRoiInput }) {
  ...
  return <div className="space-y-10 pb-8">
    <header><p className="text-xs font-medium uppercase tracking-[0.12em] text-zinc-500">Automation investment decision · {result.score}/100</p><h2 className="mt-3 text-3xl font-medium tracking-[-0.045em] text-zinc-950 sm:text-4xl dark:text-zinc-50">{result.dispositionLabel}</h2><p className="mt-4 text-[15px] leading-7 text-zinc-600 dark:text-zinc-400">{result.summary}</p><div className="mt-4 flex items-center gap-3 text-xs text-zinc-500"><span>Confidence {result.confidence}%</span><span>·</span><span>{result.baselineManualHours} baseline hours / month</span></div></header>

    <section><p className="text-xs font-medium uppercase tracking-[0.12em] text-zinc-500">Expected case</p><div className="mt-4 grid grid-cols-2 gap-3"><Metric label="First-year net value" value={money(input.currency, expected.firstYearNetValue)} /><Metric label="Payback" value={payback} /><Metric label="Net hours returned / month" value={`${expected.netHoursReturned} hrs`} detail="After review and exception fallback." /><Metric label="First-year ROI" value={roi} /><Metric label="Monthly value after run cost" value={money(input.currency, expected.monthlyNetValue)} /><Metric label="12-month build ceiling" value={money(input.currency, expected.maxBuildCostFor12MonthPayback)} detail="Approx. maximum one-time build cost for 12-month payback under this expected case." /></div></section>

    <section className="rounded-2xl bg-zinc-950 p-5 text-white dark:bg-zinc-100 dark:text-zinc-950"><p className="text-xs font-medium uppercase tracking-[0.12em] opacity-60">Conservative stress case</p><div className="mt-4 grid grid-cols-2 gap-5"><div><p className="text-xs opacity-60">First-year net</p><p className="mt-1 text-xl font-medium">{money(input.currency, conservative.firstYearNetValue)}</p></div><div><p className="text-xs opacity-60">Payback</p><p className="mt-1 text-xl font-medium">{conservative.paybackMonths === null ? 'No payback' : `${conservative.paybackMonths} mo`}</p></div></div><p className="mt-4 text-xs leading-5 opacity-70">This case assumes lower automation coverage and value capture, more review and exceptions, plus higher build and recurring cost. It is a stress test, not a prediction.</p></section>
...
export function AutomationRoiCalculator() {
  ...
  const pages: PageId[] = ['baseline', 'shape', 'cost', 'result']
  ...
  const reset = () => { setInput(DEFAULT_AUTOMATION_ROI_INPUT); setPage('baseline') }
  const next = () => setPage(pages[Math.min(pages.length - 1, index + 1)])
  const back = () => setPage(pages[Math.max(0, index - 1)])

  return <section className="roi-calculator grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden">
    <div className="pb-4"><div className="h-1.5 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-800"><div className="h-full rounded-full bg-zinc-950 transition-[width] duration-500 dark:bg-zinc-50" style={{ width: `${Math.max(7, progress)}%` }} /></div><div className="mt-3 flex items-center justify-between gap-4"><span className="text-xs font-medium text-zinc-500">{page === 'result' ? 'Analysis complete' : `${progress}% complete`}</span><button type="button" onClick={reset} className="inline-flex items-center gap-1.5 text-xs font-medium text-zinc-500 hover:text-zinc-950 dark:hover:text-zinc-50"><RotateCcwIcon className="h-3.5 w-3.5" />Start over</button></div></div>
    <div className="min-h-0 overflow-y-auto overscroll-contain pr-1 scrollbar-thin">{page === 'baseline' ? <BaselineForm input={input} update={update} /> : null}{page === 'shape' ? <ShapeForm input={input} update={update} /> : null}{page === 'cost' ? <CostForm input={input} update={update} /> : null}{page === 'result' ? <Result input={input} /> : null}</div>
    <div className="flex items-center justify-between gap-4 pt-4">{page !== 'result' ? <><button type="button" onClick={back} disabled={index === 0} className="inline-flex min-h-11 items-center gap-2 text-sm font-medium text-zinc-600 hover:text-zinc-950 disabled:invisible dark:text-zinc-400 dark:hover:text-zinc-50"><ArrowLeftIcon className="h-4 w-4" />Back</button><button type="button" onClick={next} className="inline-flex min-h-12 items-center gap-2 rounded-lg bg-zinc-950 px-5 text-sm font-medium text-white hover:opacity-85 dark:bg-zinc-50 dark:text-zinc-950">{page === 'cost' ? 'Calculate ROI' : 'Continue'}<ArrowRightIcon className="h-4 w-4" /></button></> : <button type="button" onClick={() => setPage('baseline')} className="inline-flex min-h-11 items-center gap-2 text-sm font-medium text-zinc-600 hover:text-zinc-950 dark:text-zinc-400 dark:hover:text-zinc-50"><ArrowLeftIcon className="h-4 w-4" />Edit assumptions</button>}</div>
  </section>
}
