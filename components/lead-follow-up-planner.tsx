...
function LevelButtons({ value, onChange, labels }: { value: FollowUpLevel; onChange: (value: FollowUpLevel) => void; labels: [string, string, string, string] }) {
  return <div className="grid gap-2 sm:grid-cols-2">{labels.map((label, index) => <button key={label} type="button" onClick={() => onChange(index as FollowUpLevel)} className={`min-h-12 rounded-lg px-3 py-2.5 text-left text-sm transition-colors ${value === index ? 'bg-zinc-950 text-white dark:bg-zinc-50 dark:text-zinc-950' : 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-800'}`}>{label}</button>)}</div>
}
...
function CadenceForm({ input, update }: { input: LeadFollowUpInput; update: <K extends keyof LeadFollowUpInput>(key: K, value: LeadFollowUpInput[K]) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <Field label="Channels" helper="Only select channels you can operate with appropriate eligibility, sender setup and response handling."><div className="grid grid-cols-2 gap-2">{(['email','sms','call','whatsapp'] as FollowUpChannel[]).map((channel) => <button key={channel} type="button" onClick={() => toggleChannel(channel)} className={`min-h-12 rounded-lg px-3 text-left text-sm font-medium capitalize ${input.channels.includes(channel) ? 'bg-zinc-950 text-white dark:bg-zinc-50 dark:text-zinc-950' : 'bg-zinc-100 text-zinc-700 dark:bg-zinc-900 dark:text-zinc-300'}`}>{channel === 'sms' ? 'SMS' : channel === 'whatsapp' ? 'WhatsApp' : channel}</button>)}</div></Field>
...
function Result({ input }: { input: LeadFollowUpInput }) {
  ...
  return <div className="space-y-12 pb-8">
    ...
    <section><h3 className="text-lg font-medium text-zinc-950 dark:text-zinc-50">Follow-up state model</h3><div className="mt-5 space-y-3">{result.stages.map((stage) => <article key={stage.order} className="rounded-2xl bg-zinc-100 p-5 dark:bg-zinc-900"><div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[10px] text-zinc-500">{String(stage.order).padStart(2,'0')} · {stage.timing}</p><h4 className="mt-2 text-sm font-medium text-zinc-950 dark:text-zinc-50">{stage.label}</h4></div><span className="text-right text-xs text-zinc-500">{stage.owner}</span></div><p className="mt-3 text-sm leading-6 text-zinc-600 dark:text-zinc-400">{stage.action}</p><p className="mt-2 text-xs leading-5 text-zinc-500">Exit: {stage.exit}</p></article>)}</div></section>

    {result.issues.length ? <section><h3 className="text-lg font-medium text-zinc-950 dark:text-zinc-50">Risks to fix first</h3><div className="mt-5 space-y-3">{result.issues.slice(0,6).map((issue) => <article key={issue.id} className="rounded-2xl bg-zinc-100 p-5 dark:bg-zinc-900"><p className="text-[10px] font-medium uppercase tracking-[0.12em] text-zinc-500">{issue.severity}</p><h4 className="mt-2 text-sm font-medium text-zinc-950 dark:text-zinc-50">{issue.title}</h4><p className="mt-2 text-sm leading-6 text-zinc-600 dark:text-zinc-400">{issue.impact}</p><p className="mt-3 text-sm font-medium leading-6 text-zinc-950 dark:text-zinc-50">Fix: {issue.fix}</p></article>)}</div></section> : null}

    <section className="grid gap-7 sm:grid-cols-2"><div><h3 className="text-base font-medium text-zinc-950 dark:text-zinc-50">Hard stop rules</h3><div className="mt-4 space-y-3">{result.stopRules.map((item) => <p key={item} className="text-sm leading-6 text-zinc-600 dark:text-zinc-400">• {item}</p>)}</div></div><div><h3 className="text-base font-medium text-zinc-950 dark:text-zinc-50">Safeguards</h3><div className="mt-4 space-y-3">{result.safeguards.map((item) => <p key={item} className="text-sm leading-6 text-zinc-600 dark:text-zinc-400">• {item}</p>)}</div></div></section>
    ...
    <section><h3 className="text-lg font-medium text-zinc-950 dark:text-zinc-50">Measure the system</h3><div className="mt-4 flex flex-wrap gap-2">{result.measurement.map((metric) => <span key={metric} className="rounded-md bg-zinc-100 px-2.5 py-1.5 font-mono text-[10px] text-zinc-600 dark:bg-zinc-900 dark:text-zinc-400">{metric}</span>)}</div></section>
...
export function LeadFollowUpPlanner() {
  ...
  return <div className="followup-check grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)_auto]">
