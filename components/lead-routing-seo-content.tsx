...
export function LeadRoutingSeoContent() {
  return <article className="routing-guide pb-24 pt-20 sm:pt-28">
    ...
    <section className="mt-24 rounded-3xl bg-zinc-950 px-6 py-8 text-white sm:px-8 sm:py-10 dark:bg-zinc-100 dark:text-zinc-950"><p className="text-xs font-medium uppercase tracking-[0.14em] text-zinc-400 dark:text-zinc-500">Architecture check</p><h2 className="mt-3 max-w-2xl text-2xl font-medium tracking-[-0.035em] sm:text-3xl">If routing complexity is only one part of a larger automation stack, validate the whole architecture too.</h2><div className="mt-6 flex flex-wrap gap-x-6 gap-y-3 text-sm font-medium"><a href={`${SITE.origin}/tools/automation-architecture-advisor`} className="transition-opacity hover:opacity-70">Automation Architecture Advisor →</a><a href={`${SITE.origin}/tools/crm-automation-health-check`} className="transition-opacity hover:opacity-70">CRM Automation Health Check →</a><a href={`${SITE.origin}/crm-automation`} className="transition-opacity hover:opacity-70">CRM automation guide →</a></div></section>
  </article>
}
