...
test('strong standardized onboarding scores as ready without invented critical leaks', () => {
  const result = analyzeOnboarding(input({
    crmId: 'hubspot', contractAppId: 'docusign', billingAppId: 'stripe', projectAppId: 'clickup', intakeAppId: 'typeform',
    communicationAppId: 'gmail', fileAppId: 'google-drive', triggerMode: 'compound', requireWon: true, requireSigned: true,
    requirePaid: true, processRepeatability: 3, handoffData: 3, intakeQuality: 3, accessComplexity: 'multi', accessTracking: 3,
    workspaceTemplate: 3, clientWelcome: 3, kickoffScheduling: 3, ownerAssignment: 3, reminders: 3, readinessGate: 3,
    duplicateProtection: 3, exceptionHandling: 3, monitoring: 3,
  }))
...
test('payment prerequisite without an authoritative billing system is called out', () => {
  const result = analyzeOnboarding(input({ crmId: 'hubspot', triggerMode: 'compound', requireWon: true, requirePaid: true, billingAppId: null }))
...
test('variable bespoke onboarding recommends standardization before deeper automation', () => {
  const result = analyzeOnboarding(input({ crmId: 'salesforce', monthlyClients: 40, serviceVariants: 12, processRepeatability: 0, workspaceTemplate: 0, readinessGate: 0, exceptionHandling: 0 }))
...
test('external payment or contract events require duplicate-safe creation', () => {
  const result = analyzeOnboarding(input({ crmId: 'gohighlevel', billingAppId: 'stripe', triggerMode: 'payment-received', requirePaid: true, duplicateProtection: 0 }))
