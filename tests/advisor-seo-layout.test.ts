...
test('advisor owns the first viewport with one explicit centered workspace', () => {
  ...
  assert.match(routeFrame, /lead-routing-rules-builder/)
  ...
  assert.match(css, /\.advisor-viewport,[\s\S]*\.routing-viewport\s*\{[^}]*height:\s*100dvh/)
  assert.match(css, /\.advisor-workspace,[\s\S]*\.routing-workspace\s*\{[^}]*place-items:\s*stretch center/)
