...
test('interactive tool routes use a genuinely wide application frame', () => {
  ...
  assert.match(css, /\.tool-app-viewport\s*\{[^}]*width:\s*100%/)
  assert.match(css, /\.tool-app-viewport\s*\{[^}]*max-width:\s*1600px/)
  assert.doesNotMatch(css, /\.tool-app-viewport\s*\{[^}]*left:\s*50%/)
  assert.doesNotMatch(css, /\.tool-app-viewport\s*\{[^}]*translate:\s*-50% 0/)
  ...
  assert.match(css, /\.advisor-workspace > section > div:first-child,[\s\S]*width:\s*min\(100%, 1400px\)/)
  assert.match(css, /body\s*\{[^}]*overflow-x:\s*clip/)
})

test('tool layout contains horizontal overflow instead of clipping content off-canvas', () => {
  assert.match(css, /\.advisor-workspace\s*,[\s\S]*overflow:\s*hidden/)
