...
test('advisor owns the first viewport with one explicit centered workspace', () => {
  ...
  assert.match(css, /\.advisor-viewport,[\s\S]*\.crm-health-viewport\s*\{[^}]*width:\s*100%/)
  assert.match(css, /\.advisor-viewport,[\s\S]*\.crm-health-viewport\s*\{[^}]*height:\s*100dvh/)
  assert.match(css, /\.advisor-workspace,[\s\S]*\.crm-health-workspace\s*\{[^}]*place-items:\s*stretch center/)
