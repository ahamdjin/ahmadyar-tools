...
HubSpot supports workflow-based record-owner assignment and rotation. Current documentation describes equal distribution between selected users/teams, plus additional assignment modes for some objects. It also documents important operational caveats, including seat requirements and owner-sync interactions with Salesforce.
...
1. **Capture comes before automation.** Important lead sources need to create or update the intended CRM record with the context required downstream. A workflow cannot repair a lead that never entered the operating system.
